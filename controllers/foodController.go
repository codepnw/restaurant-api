...
func GetFoods() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		matchState := bson.D{{Key: "$match", Value: bson.D{{}}}}
		groupState := bson.D{{
		...
		projectStage := bson.D{{
			Key: "$project", Value: bson.D{
				...
				{Key: "food_items", Value: bson.D{{Key: "$slice", Value: []any{"$data", startIndex, recordPerpage}}}},
			},
		}}

		result, err := foodCollection.Aggregate(ctx, mongo.Pipeline{matchState, groupState, projectStage})
		defer cancel()
